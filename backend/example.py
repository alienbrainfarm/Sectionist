...
def analyze_song_structure(y, sr):
    ...
    boundary_times = detect_boundaries_frame_based(chroma, rms, spectral_centroid, frame_times, duration)
    
    # Label sections based on their characteristics  
    sections = label_sections_frame_based(boundary_times, chroma, rms, spectral_centroid, frame_times)
    
    return sections


def detect_boundaries_frame_based(chroma, rms, spectral_centroid, frame_times, duration):
    """
    ...
    for i in range(frames_per_window, chroma.shape[1] - frames_per_window, frames_per_step):
        ...
        after_centroid = spectral_centroid[:, i:min(spectral_centroid.shape[1], i + frames_per_window)]
        
        # Calculate differences
        if (before_chroma.size > 0 and after_chroma.size > 0 and 
            before_rms.size > 0 and after_rms.size > 0):
            
            chroma_diff = np.linalg.norm(np.mean(before_chroma, axis=1) - np.mean(after_chroma, axis=1))
            ...
            if boundary_score > 0.3:  # Adjusted threshold
                boundary_time = frame_times[i] if i < len(frame_times) else frame_times[-1]
...
def label_sections_frame_based(boundary_times, chroma, rms, spectral_centroid, frame_times):
    """
    ...
    for i in range(len(boundary_times) - 1):
        ...
        if end_frame > start_frame and start_frame < chroma.shape[1]:
            ...
            segment_centroid = spectral_centroid[:, start_frame:min(end_frame, spectral_centroid.shape[1])]
            
            # Calculate characteristics
            energy_level = float(np.mean(segment_rms)) if segment_rms.size > 0 else 0.3
            chroma_stability = 1.0 / (1.0 + np.var(segment_chroma)) if segment_chroma.size > 0 else 0.5
            brightness = float(np.mean(segment_centroid)) if segment_centroid.size > 0 else 1000
        else:
            energy_level = 0.3
...
def analyze_audio_file(file_path):
    ...
    return {
        ...
        "tempo": round(float(tempo.item()) if hasattr(tempo, 'item') else float(tempo), 1),
        "key": detected_key,
        "sections": sections,
